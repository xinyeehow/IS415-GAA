...
**Filtering Osun state in Nigeria**

```{r}
wp_nga_osun <- wp_nga %>% filter(`clean_country_name` == "Nigeria", `clean_adm1` == "Osun")
```
...
```{r}
wp_nga_osun = st_as_sf(wp_nga_osun, coords = c("lat_deg", "lon_deg"),crs=4326)
wp_nga_osun

wp_sf_osun <- st_sf(wp_nga_osun, crs=4326)
wp_sf_osun
```
...
```{r}
freq(data = wp_sf_osun, input = 'status_clean')
```
...
```{r}
write_rds(wp_sf_osun, "data/rds/wp_sf_osun.rds")
```
...
**Categorising functional and nonfunctional waterpoints**
...
**Filtering functional and nonfunctional waterpoints**
...
As we can observe from the map plotted above, waterpoints with their LCLQ value of 1 (in yellow) are displayed. We can conclude that our null hypothesis is true; that functional waterpoints are not co-located with nonfunctional water points in Osun state (Nigeria).
